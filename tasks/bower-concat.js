...
module.exports = function(grunt) {
    ...
    grunt.registerMultiTask('bower_concat', 'Concatenate installed Bower packages.', function() {
        ...
        function findMainFiles(name, component) {
            ...
            var componentFolder = grunt.file.isFile(mainFiles[0]) ? path.dirname(mainFiles[0]) : mainFiles[0];
            if (mains[name]) {
                var manualMainFiles = ensureArray(mains[name]);
                manualMainFiles = _.map(manualMainFiles, function(filepath) {
                    return path.join(bowerOptions.relative ? bower.config.cwd : '', path.join(componentFolder, filepath));
                });
            ...
            mainFiles = _.map(mainFiles, function(filepath) {
                return path.join(bowerOptions.relative ? bower.config.cwd : '', filepath);
            });
            ...
            var jsFiles = grunt.file.expand(path.join(bowerOptions.relative ? bower.config.cwd : '', componentFolder, '*.js'));
